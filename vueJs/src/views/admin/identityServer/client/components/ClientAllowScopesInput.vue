...
@Component({
  name: 'ClientAllowScopesInput'
})
export default class extends Vue {
  @Prop({ default: () => new Array<ClientScope>() })
  private value!: any[]
  ...
  // private onValueChanged(val: ClientScope[]) {
  //   const tmpTags = new Array<string>()
  //   val.forEach(v => {
  //     tmpTags.push(v.scope)
  //   })
  ...
  private addTag(tag: any) {
    ...
    tag = tag.trim()
    if (!this.value.every(data => data[this.key] === tag)) {
      const newScope = new ClientScope()
      newScope.scope = tag
      this.value.push(newScope)
      return true
    }
    return false
  }
