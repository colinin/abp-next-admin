  ...
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{row}">
          <el-button
            :disabled="!checkPermission(['AbpIdentityServer.ApiScopes.Update'])"
            size="mini"
            type="primary"
            @click="onShowEditForm(row.id)"
          >
            {{ $t('AbpIdentityServer.ApiScopes:Edit') }}
          </el-button>
          <el-button
            :disabled="!checkPermission(['AbpIdentityServer.ApiScopes.Delete'])"
            size="mini"
            type="danger"
            @click="onDeleted(row.id, row.name)"
          >
            {{ $t('AbpIdentityServer.ApiScopes:Delete') }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import ApiScopeCreateOrEditForm from './components/ApiScopeCreateOrEditForm.vue'
import ApiScopeService, { ApiScopeGetByPaged } from '@/api/api-scopes'
...
export default class extends mixins(DataListMiXin) {
  ...
  public dataFilter = new ApiScopeGetByPaged()
  ...
  protected getPagedList(filter: any) {
    return ApiScopeService.getPagedList(filter)
  }
  ...
  private onDeleted(id: string) {
    this.$confirm(this.l('AbpIdentityServer.ApiScopes:Delete'),
      this.l('AbpUi.AreYouSure'), {
        callback: (action) => {
          if (action === 'confirm') {
            ApiScopeService.delete(id)
              .then(() => {
