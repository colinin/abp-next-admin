...
public class Program
{
    public async static Task<int> Main(string[] args)
    {
        try
        {
            ...
            {
                IdentityServerModule.ApplicationName = Environment.GetEnvironmentVariable("APPLICATION_NAME")
                    ?? IdentityServerModule.ApplicationName;
                options.ApplicationName = IdentityServerModule.ApplicationName;
                // 从环境变量取用户机密配置, 适用于容器测试
                options.Configuration.UserSecretsId = Environment.GetEnvironmentVariable("APPLICATION_USER_SECRETS_ID");
                // 如果容器没有指定用户机密, 从项目读取
                options.Configuration.UserSecretsAssembly = typeof(IdentityServerModule).Assembly;
