    ...
    public class SignalRNotificationPublishProvider : NotificationPublishProvider
    {
        ...
        protected override async Task PublishAsync(NotificationInfo notification, IEnumerable<UserIdentifier> identifiers, CancellationToken cancellationToken = default)
        {
            if (identifiers?.Count() == 0)
            {
                ...
                var signalRGroup = _hubContext.Clients.Group(groupName);
                // 租户通知群发
                Logger.LogDebug($"Found a signalr group, begin sending notifications");
                await signalRGroup.SendAsync(_options.MethodName, notification, cancellationToken);
            }
            else
            {
                try
                {
                    ...
                    Logger.LogDebug($"Found a signalr client, begin sending notifications");
