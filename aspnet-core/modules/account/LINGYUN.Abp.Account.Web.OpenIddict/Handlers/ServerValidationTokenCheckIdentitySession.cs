...
public class ServerValidationTokenCheckIdentitySession : IOpenIddictServerHandler<OpenIddictServerEvents.ValidateTokenContext>
{
    ...
    {
        Logger.LogDebug("Server Validate Token: {endpointType} - {requestUri}", context.EndpointType, context.RequestUri);
        ...
        {
            Logger.LogWarning("SessionId is null for endpoint '{endpointType}', skipping session validation.", context.EndpointType);
            return;
        }
